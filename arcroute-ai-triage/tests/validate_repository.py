...
def validate_workflow():
    ...
    if version_id and not re.fullmatch(
        r"[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}",
        version_id,
    ):
        fail("Workflow versionId must be absent or a UUID generated by n8n")
...
def validate_no_real_secrets():
    for path in ROOT.rglob("*"):
        ...
        if DEPRECATED_MODEL in text:
            fail(f"Deprecated model {DEPRECATED_MODEL} found in {path.relative_to(ROOT)}")
...
def main():
    ...
    for record in outputs:
        ...
        if record["model_name"] != DEFAULT_MODEL:
            fail(f"{record['request_id']}: model_name should be {DEFAULT_MODEL}")
